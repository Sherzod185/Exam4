...
function rating(el) {
  if (el.rating.rate < 2.4) {
    el.rating.rate = "<i class='bx iconcha bxs-star bx-tada' style='color:#e3ee0e' ></i><i class='bx bxs-star iconcha bx-tada' style='color:#e3ee0e' ></i><i class='bx iconcha bx-star '></i><i class='bx iconcha bx-star '></i><i class='bx iconcha bx-star '></i>"
  }
  else if (el.rating.rate < 3.4) {
    el.rating.rate = "<i class='bx iconcha bxs-star bx-tada' style='color:#e3ee0e' ></i><i class='bx iconcha bxs-star bx-tada' style='color:#e3ee0e' ></i><i class='bx iconcha bxs-star bx-tada' style='color:#e3ee0e' ></i><i class='bx iconcha bx-star '></i><i class='bx iconcha bx-star '></i>"
  }
  else if (el.rating.rate < 4.4) {
    el.rating.rate = "<i class='bx iconcha bxs-star bx-tada' style='color:#e3ee0e' ></i><i class='bx iconcha bxs-star bx-tada' style='color:#e3ee0e' ></i><i class='bx iconcha bxs-star bx-tada' style='color:#e3ee0e' ></i><i class='bx iconcha bxs-star bx-tada' style='color:#e3ee0e' ></i><i class='bx iconcha bx-star '></i>"
  }
  else {
    el.rating.rate = "<i class='bx iconcha bxs-star bx-tada' style='color:#e3ee0e' ></i><i class='bx iconcha bxs-star bx-tada' style='color:#e3ee0e' ></i><i class='bx iconcha bxs-star bx-tada' style='color:#e3ee0e' ></i><i class='bx iconcha bxs-star bx-tada' style='color:#e3ee0e' ></i><i class='bx iconcha bxs-star bx-tada' style='color:#e3ee0e' ></i>"
  }
...
function apiMore() {
  ...
  <span class="price">$ ${product.price}🤔 <p class="sale">24% SALE OF</p> $ ${(product.price * 0.76).toFixed(2)}🤩 </span>
...
cartcont.addEventListener("click", e => {
  if (e.target.className == "b2") {
    ...
    let innerEl = e.target.parentElement.previousElementSibling.innerHTML.split("")
    let secArr = innerEl.splice(0, innerEl.length - 1).join("")
    e.target.parentElement.nextElementSibling.innerHTML = (sum * Number(secArr)).toFixed(2) + "$";
...
function apiProduct() {
  ...
  fetch('https://fakestoreapi.com/products')
    ...
    <i class='bx  bx-shopping-bag add-cart' id="${product.id}"></i>
        `
        if (product.id == 1 || product.id == 2 || product.id == 3 || product.id == 4 || product.id == 5 || product.id == 6 || product.id == 7 || product.id == 8) {
          shopcontent.appendChild(div)
        }
      })
    });
...
async function fetchData(url) {
  await fetch(url).then(res => res.json()).then(data => { tovar = data; renderCards(data) })
}

function renderCards(data) {
  $a(".shop-content").innerHTML = ""
  data.forEach(product => {
    let card = $$("div", "card d-inline-block product-box", `
    <div class="product-img">
    <img src="${product.image}" alt="watch1" class="product-img">
  </div>
  <h3 class="product-title">
...
$a(".searchProduct").addEventListener("input", function (e) {
  let searchData = tovar.filter(el => el.title.toLowerCase().includes($a(".searchProduct").value.toLowerCase()) || (el.title && el.title[0].toLowerCase().includes($a(".searchProduct").value.toLowerCase())))

  renderCards(searchData)
})

$a(".searchProduct").addEventListener("click", () => fetchData("https://fakestoreapi.com/products"))
